...
**Chief Architect, Parsley Health.** I founded and led the tiger team responsible for emerging products and owned this initiative end to end: stakeholder discovery, the technology evaluation and selection, the technical design, and delivery leadership through four phase-gated cycles to production launch.
 
Specifically, I owned:
 
- The technology-readiness evaluation and the decision to self-host rather than use a proprietary API
- The system architecture, the AI pipeline design, and the Technical Design Document
- The compliance architecture, designed jointly with the compliance team
- Delivery leadership for a team of 3 engineers and 1 QA, and coordination with clinical, product, data, and DevOps stakeholders
- Success metric definition with the product team, and the reporting model for leadership
Timeline: approximately twelve months from stakeholder alignment (February 2023) to production launch (January 2024). Infrastructure implementation was delegated to the DevOps team under a design I authored and reviewed. Architecture was reviewed and signed off by the CTO.
...
- Built the LLM service in **Python** using HuggingFace `transformers` for inference and `bitsandbytes` for 4-bit quantization
- Implemented **Retrieval-Augmented Generation (RAG)** to ground recommendations in clinical evidence:
  - Primary source: AMA (American Medical Association) clinical standards
...
- Reviewed architecture and obtained signoff from the CTO
---
...
```mermaid
graph LR
    A[Lab Results<br/>Ingestion] --> B[GCP Cloud Pub/Sub<br/>Event Bus]
    B --> C[AI Processing<br/>Service - Python/K8s]
    C --> D[Recommendation<br/>Engine]
    D --> E[Web / Mobile App]
    B --> F[Datadog<br/>Telemetry Subscription]
    B --> G[BigQuery<br/>Data Lake Subscription]
    D --> H[Health Coach<br/>Review Dashboard]
```
...
```mermaid
flowchart TD
    A[Raw Lab Results<br/>PHI + PII] --> B[De-identification<br/>PII stripped, UUID retained]
    B --> C[Clinical Context Injection<br/>RAG on AMA guidelines +<br/>clinical knowledge base]
    C --> D[LLM Inference<br/>Llama 2 - quantized]
    D --> E[Output Validation<br/>& Safety Checks]
    E --> F[Health Goals &<br/>Lifestyle Recommendations]
    F --> G[Patient-Facing<br/>Web / Mobile Delivery]
    E --> H[Health Coach<br/>Review Flag]
```
...
- Team composition: 3 engineers (AI service, platform, and integrations) and 1 QA; DevOps infrastructure delegated to the DevOps team under joint design
---
...
- Ensured all GCP services were configured within a VPC with private service access. No workload was exposed to the public internet.
- HIPAA compliance validated via VPC Service Controls, CMEK (Customer-Managed Encryption Keys) on BigQuery and Cloud Storage, and audit logging enabled across all data-plane services
...
- Implemented a multi-turn conversational layer in Python, maintaining session context so patients could ask follow-up questions referencing prior messages (for example, "Why do I need to reduce sodium?" following a dietary recommendation)
- RAG retrieval scoped to the patient's specific recommendation set and relevant AMA guideline excerpts. The coach answered questions grounded in the patient's own data and clinical evidence, not generic health information.
...
The virtual health coach shipped as a pilot at the close of Phase 4. I departed at the end of the pilot period, so the results below reflect the core recommendation pipeline rather than the conversational layer.
 
---
 
## Results & Outcomes
 
> *Note: Figures below are rounded. Precise internal values are subject to NDA.*
...
## Tradeoffs & What I'd Change
 
Every decision here carried a cost. The ones worth naming:
 
**Self-hosting over a proprietary API.** Self-hosting was the right call given HIPAA data residency requirements and the state of BAA availability for hosted models in early 2023. It cost us inference performance, GPU infrastructure overhead, and engineering time that a hosted API would have absorbed. If I were starting this today, the evaluation would be genuinely closer, and I would re-run the technology-readiness matrix rather than assume the same answer.
 
**Llama 2 as the foundation model.** It was the most capable openly licensed option at project inception. The open-weight landscape has moved substantially since, and a model selected today would likely be both smaller and stronger. Building the inference layer behind a service boundary was deliberate for exactly this reason: the model is replaceable without touching the pipeline.
 
**4-bit quantization.** This traded some output quality for a meaningful reduction in GPU cost and latency. At our volume it was the correct trade, and the health coach review layer caught what quantization cost us. At materially higher volume, or in a workflow without human review in the loop, I would revisit it.
 
**RAG rather than fine-tuning.** RAG kept the clinical corpus auditable and updatable without a retrain cycle, which mattered for compliance and for the correction feedback loop. It also meant recommendation quality was bounded by retrieval quality, and retrieval tuning consumed more of the schedule than initially estimated.
 
**The 87% approval rate is also a 13% correction rate.** Roughly one in eight recommendations needed human modification. That was acceptable for an MVP with mandatory review, and it drove the corpus improvement loop, but it is the number I would want to see trending before removing any human checkpoint. Automating clinical review entirely was never the goal, and I would resist framing it that way.
 
**De-identification limited personalization.** Stripping PII at the pipeline boundary was non-negotiable for compliance, and it meant the model reasoned over lab values without full clinical history context. Some recommendation nuance was lost to that constraint. It was the right trade, but it was a real one.
...
```mermaid
flowchart LR
    A[AI Output] --> B[Health Coach<br/>Review Dashboard]
    B --> C{Approved?}
    C -- Yes --> D[Delivered to Patient]
    C -- No --> E[Correction &<br/>Annotation]
    E --> F[RAG Corpus<br/>Augmentation]
    F --> G[Quarterly Retrain<br/>Evaluation]
    G --> A
    A --> H[Datadog<br/>Monitoring]
    H --> I[Drift Alerts]
    I --> G
```
...
**Michael St.Claire** · [mstclaire@five07.com](mailto:mstclaire@five07.com) · [LinkedIn](https://www.linkedin.com/in/themichaelstclaire)
